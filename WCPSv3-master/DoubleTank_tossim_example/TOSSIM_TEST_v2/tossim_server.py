...
def stringTofloatToInt(num):
    try:
        tmp = float(num)
        return int(tmp)
    except:
        print "can't change str to int"

def ReviseStr(string):
    tmp1 = string.replace('[','')
    tmp2 = tmp1.replace(']','')
    list = tmp2.split(',')
    return list
...
print "ReviseStr:",ReviseStr(dummy)
tmp = ReviseStr(dummy)
Da=[]
for i in tmp:
    Da.append(stringTofloatToInt(i))
print Da
'''
...
while True:
    ...
    try:
        while True:
            ...
            #print "data:",data
            #print "data type:",type(stringTofloatToInt(data))
            if type(stringTofloatToInt(data))== type(1):    #check data type is int
                #print "Event_flag trans"
                t.setEvent(stringTofloatToInt(data))
            if data:
